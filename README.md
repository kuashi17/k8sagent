...
현재 기본 planner는 외부 LLM API 없이 동작하는 `mock` 모드입니다. Agent는 요구사항을 요약하고, `knowledge-base` 문서를 검색한 뒤, 기존 CLI 도구를 Tool wrapper로 호출합니다.

현재 `mock` planner는 규칙 기반이지만 LangChain Agent 구조 위에서 동작합니다. 향후 ChatOpenAI 또는 로컬 LLM planner로 교체할 수 있도록 `mock`, `llm`, `local` planner 타입을 분리해 두었습니다.
...
LLM planner를 사용하는 requirement 기반 dry-run:
