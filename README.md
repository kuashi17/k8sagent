...
기본 화면에는 자연어 요구사항과 `계획 확인하기`만 표시됩니다. Profile,
run-level, kind 배포, 기존 프로젝트 재사용은 고급 설정에 있고, Safety,
Evidence, Recovery, stdout/stderr 원문은 작업 결과의 개발자 상세 정보에
접혀 있습니다.

Web UI의 기본 흐름:

1. 자연어 요구사항을 입력하고 계획을 확인합니다.
2. 관리 리소스, 완료 단계, 생성 대상과 경고를 검토합니다.
3. 계획 결과에서 `계획 승인하고 생성하기`를 눌러 실제 생성을 승인합니다.
