...
| `agent/tools/e2e_runner.py` | kind e2e 실행 | TrainingJob profile 성격이 강함 | TrainingJob CRD, Job 이름, Pod label, GPU/PVC/env 검증 |
...
- `e2e_runner.py`가 profile 파일을 읽을 수 있는 구조 설계
- TrainingJob Job spec validation 규칙을 profile 파일로 이전
