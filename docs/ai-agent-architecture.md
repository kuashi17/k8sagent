...
## LLM planner와 mock fallback

심사/시연의 기본 planner는 `llm`이다. `llm` planner는 실제 Chat Model 호출을 수행하고, RAG 검색 결과를 입력으로 받아 요구사항 요약, 부족 정보 판단, profile 추천, Tool 호출 계획, 위험 요소, 다음 조치를 JSON으로 생성한다.

`mock` planner는 최종 AI 기능이 아니라 개발/테스트/오프라인 fallback용 안전장치다.

mock planner를 유지하는 이유:

- API key가 없어도 파이프라인 테스트 가능
- 결과가 일정하여 회귀 테스트에 유리
- 정해진 dry-run 순서만 실행하므로 안전
- 시연 중 네트워크, rate limit, 로컬 모델 장애가 발생해도 최소 흐름 유지
