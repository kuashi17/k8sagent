...
## 오류 기록은 다음 작업에 자동 사용되는가?

자동으로 사용되지는 않습니다. 오류는 로컬 로그에 저장되고 현재 작업의 원인 분석과 복구 안내에 사용되지만, Local LLM이 오류를 자동 학습하거나 다음 Operator 요청의 지식으로 추가하지 않습니다.

| 범위 | 사용 방식 |
| --- | --- |
| 현재 실패 작업 | errorCode와 `failure-context.json`을 기준으로 원인과 다음 조치 생성 |
| 나중에 같은 로그 확인 | Web UI의 실패 로그 분석에서 사용자가 해당 작업을 선택해 다시 분석 가능 |
| 다음 요구사항 계획 | 이전 실패 로그를 자동으로 입력하거나 계획 cache에 저장하지 않음 |
| 장기 재사용 | 검토된 오류만 Error Registry, knowledge-base와 회귀 테스트에 코드 변경으로 추가 |

이 구조는 한 번의 잘못된 로그나 모델 추측이 이후 모든 계획에 자동으로 영향을 주는 것을 방지합니다. 반복되는 오류를 제품 지식으로 사용하려면 원인과 해결 방법을 확인한 뒤 명시적으로 등록해야 합니다.
